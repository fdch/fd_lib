...
function deps_string() {
	...
	local ds=''
	local i=0
	if [[ $dn -ge 1 ]]; then # more than one dependencies
		for i in ${@}; do
			ds="${ds}${i}:" # append dependency with colon
		done
		ds="${ds%:}" # just remove trailing colon
		echo ${dn} ${ds}
	elif [[ $dn == 1 ]]; then # exactly one dependency
		ds=$1
		echo 1 ${ds}
	else # no deps
		ds='0'
...
function get_class_names() {
	...
	for ((i = 0; i < ${#a[@]}; i += 2)); do
		...
		# echo $pat >&2
		if grep -q -c "$pat" /tmp/vanilla; then
...
function get_vanilla_objects() {
	...
	for i in $(ls ${EXTRA}); do
		if [[ -d $i ]]; then # directories in 'extra' dir are pdfiles
			basename $i .pd >>/tmp/vnl
		elif grep -q -v "-help.pd" <<<"$i"; then # ignore helpfiles
			if grep -q ".pd" <<<"$i"; then          # some pdfiles are inside 'extra' dir
				basename $i .pd >>/tmp/vnl
			fi
		fi
	done
...
function get_all_pd_files() {
	local path=$1
	for i in $path/*.pd; do
		if grep -q -v "-help.pd" <<<"$i"; then ## ignore helpfiles
			if grep -q -v "test" <<<"$i"; then    ## ignore test files
				echo $i
			fi
		fi
	done
