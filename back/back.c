/* This is a hack to the x_scalar.c object in Pd-0.47-1 */
...
/* Removed until I figure out how to use this... */
/*
...
void scroll_xaxis(t_glist *x, t_float f)
{
...
void scroll_yaxis(t_glist *x, t_float f)
{
	if (!(glist_isvisible(x))) canvas_vis(x, 1);
	if (x)
...
void back_define_cursor(t_glist *x, t_symbol *s)
{
	canvas_vis(x,1);
	sys_vgui(".x%lx.c configure -cursor %s\n",x,s->s_name);
}

void back_define_background(t_glist *x, t_symbol *s)
{
	canvas_vis(x,1);
	sys_vgui(".x%lx.c configure -background %s\n", x,s->s_name);
}

void back_define_text(t_glist *x, t_floatarg xpos, t_floatarg ypos, t_symbol *s)
{
	canvas_vis(x,1);
	sys_vgui (".x%lx.c create text %f %f -text %s\n", x,xpos, ypos, s->s_name);
}

void back_define_shape(t_glist *x, t_symbol *s, t_floatarg x1, t_floatarg y1, t_floatarg x2, t_floatarg y2)
{
	canvas_vis(x,1);
	sys_vgui (".x%lx.c create %s %f %f %f %f\n", x,s->s_name,x1,y1,x2,y2);
}

void back_define_delete(t_glist *x)
{
	canvas_vis(x,1);
...
/* ---------------- canvas functions -------------------- */
void back_define_vis(t_glist *x, t_floatarg f){canvas_vis(x, f);}

void back_define_clear(t_glist *x)
{
	t_symbol *templatesym = &s_float;
	glist_clear(x);
	putscalar(x, templatesym);
...
void back_define_sort(t_glist *x){glist_sort(x);}

/* ---------------- read, write, merge -------------------- */
void back_define_read(t_glist *x, t_symbol *filename, t_symbol *format)
{
	glist_read(x, filename, format);
}

void back_define_mergefile(t_glist *x, t_symbol *filename, t_symbol *format)
{
	glist_mergefile(x, filename, format);
}

void back_define_write(t_glist *x, t_symbol *filename, t_symbol *format)
{
	glist_back_write(x, filename, format);
}

/* ---------------- memory -------------------- */
void back_define_free(t_glist *x)
{
