...
for i in $HELPFILES
do
	# ignoring patches without helpfiles...
	PNAME=`echo $i | cut -f2 -d/`
	PATCHFILES+=" ./$PNAME/$PNAME.pd"
done
TESTFILES=$(find . -name "*-test.pd" | awk -v RS="" '{gsub (/\n/," ")}1')
READMEFILES=$(find . -name "README.txt" -name "README.md"| awk -v RS="" '{gsub (/\n/," ")}1')
for i in $CLASSSOURCES, $HELPFILES, $TESTFILES, $READMEFILES, $PATCHFILES
do 
((FOUND++))
done
...
datafiles = $HELPFILES $TESTFILES $READMEFILES $PATCHFILES
