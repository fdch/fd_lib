...
static void connect_disconnect(t_connect *x, t_floatarg index1, t_floatarg outno,
                            t_floatarg index2, t_floatarg inno)
{
  if(!connect_settargetcanvas(x, x->x_tgtsym))
    return;
  if(!glist_isvisible(x->x_tgtcnv))
...
void connect_setup() {
  connect_class = class_new(gensym("connect"), (t_newmethod)connect_new,0,
                            sizeof(t_connect), CLASS_DEFAULT, A_DEFSYM, 0);
  class_addmethod(connect_class, (t_method)connect_disconnect,
                  gensym("disconnect"), A_FLOAT, A_FLOAT, A_FLOAT, A_FLOAT, A_NULL);
  class_addmethod(connect_class, (t_method)connect_connect,
