...
static void scroll_moveto(t_scroll *x, int d, float f)
{
  if (!sys_havegui())
	...
	if (!(glist_isvisible(x->canvas)))
    canvas_vis(x->canvas, 1);
...
void scroll_setup()
{
	...
	class_addmethod(scroll_class, (t_method)scroll_screenx, gensym("screenx"), A_FLOAT, 0);
	class_addmethod(scroll_class, (t_method)scroll_screeny, gensym("screeny"), A_FLOAT, 0);
}
