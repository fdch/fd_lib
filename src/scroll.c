...
typedef struct scroll
{
  ...
  int       x_norm;    /* flag to user normalized coordinates (1) or not (0) (default=0) */
} t_scroll;
...
static void scroll_page(t_scroll *x, t_floatarg f)
{ x->x_page = !!(int)f; }

static void scroll_norm(t_scroll *x, t_floatarg f)
{ x->x_norm = !!(int)f; }
...
static void scroll_screenx(t_scroll *x)
{ scroll_get_screenx(x); }

static void scroll_screeny(t_scroll *x)
{ scroll_get_screeny(x); }
...
static void scroll_moveto(t_scroll *x, int d, t_floatarg f)
{
  ...
  pdgui_vmess(0, "cr sf", x->x_canvas, moveto_direction,
              "moveto", famount);
}
...
static void scroll_scroll(t_scroll *x, int d, int f)
{
  ...
  pdgui_vmess(0, "cr sis", x->x_canvas, scroll_direction,
              "scroll", (int)f, scroll_type);
}

static void scroll_xaxis(t_scroll *x, t_floatarg f)
{ scroll_scroll(x, 1, f); }
...
void scroll_setup()
{
	...
	class_addmethod(scroll_class, (t_method)scroll_xaxis,
                 gensym("xaxis"), A_FLOAT, 0);
	class_addmethod(scroll_class, (t_method)scroll_yaxis,
                 gensym("yaxis"), A_FLOAT, 0);
	class_addmethod(scroll_class, (t_method)scroll_xmoveto,
                 gensym("xpos"), A_FLOAT, 0);
	class_addmethod(scroll_class, (t_method)scroll_ymoveto,
                 gensym("ypos"), A_FLOAT, 0);
	class_addmethod(scroll_class, (t_method)scroll_page,
                 gensym("page"), A_FLOAT, 0);
	class_addmethod(scroll_class, (t_method)scroll_norm,
                 gensym("norm"), A_FLOAT, 0);
	class_addmethod(scroll_class, (t_method)scroll_screenx,
                 gensym("screenx"), A_FLOAT, 0);
	class_addmethod(scroll_class, (t_method)scroll_screeny,
                 gensym("screeny"), A_FLOAT, 0);
}
