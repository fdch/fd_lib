...
// Needed for qsort.  See
// http://w...content-available-to-author-only...s.com/reference/cstdlib/qsort/
t_int compare(const void *a, const void *b) { return (*(int *)a - *(int *)b); }
...
/*
        ...
        genrand() and sgenrand() taken from "M. Matsumoto and T. Nishimura
   (1996)
...
int is_carriage_return_enabled(int *argc, t_atom **argv) {
  int ac = *argc;
  t_atom *av = *argv;
  while (ac && av->a_type == A_SYMBOL && *atom_getsymbol(av)->s_name == '-') {
    if (!strcmp(atom_getsymbol(av)->s_name, "-c")) {
      return (1);
    } else {
      pd_error(0, "text write: unknown flag ...");
      postatom(ac, av);
      endpost();
    }
    ac--;
