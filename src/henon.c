...
{
  henon_class = class_new(gensym("henon"), (t_newmethod)henon_new, 0,
                          sizeof(t_henon), CLASS_DEFAULT,
                          A_DEFFLOAT, A_DEFFLOAT, A_DEFFLOAT, A_NULL);
  class_addbang(henon_class, henon_bang);
  class_addmethod(henon_class, (t_method)henon_print,
                  gensym("print"), A_FLOAT, A_NULL);
  class_addmethod(henon_class, (t_method)henon_reset,
                  gensym("reset"), A_NULL);
  class_addmethod(henon_class, (t_method)henon_reset,
                  gensym("clear"), A_NULL);
}
