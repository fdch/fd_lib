...
static void cuadratic_step(double *x, double c)
{ *x = ((*x) * (*x)) + c; }
...
static void cuadratic_c(t_cuadratic *x, t_float f)
{ x->x_c = (double)f; }

static void cuadratic_x(t_cuadratic *x, t_float f)
{ x->x_x = (double)f; }
...
void cuadratic_setup(void) {
  cuadratic_class = class_new(gensym("cuadratic"), (t_newmethod)cuadratic_new, 0,
                              sizeof(t_cuadratic), CLASS_DEFAULT, A_DEFFLOAT, A_DEFFLOAT, A_NULL);
  class_addbang(cuadratic_class, cuadratic_bang);
  class_addfloat(cuadratic_class, cuadratic_float);
  class_addmethod(cuadratic_class, (t_method)cuadratic_c,
                  gensym("c"), A_FLOAT, A_NULL);
  class_addmethod(cuadratic_class, (t_method)cuadratic_x,
                  gensym("x"), A_FLOAT, A_NULL);
  class_addmethod(cuadratic_class, (t_method)cuadratic_reset,
                  gensym("reset"), A_NULL);
}
