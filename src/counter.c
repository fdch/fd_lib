...
static void counter_m(t_counter *x, t_floatarg f)
{ x->x_m = (int)f; }

static void counter_r(t_counter *x, t_floatarg f)
{ x->x_r = (int)fabsf(f); }

static void counter_i(t_counter *x, t_floatarg f)
{ x->x_i = f; }
...
void counter_setup(void)
{
  ...
  class_addcreator((t_newmethod)counter_new,
                   gensym("c"), A_GIMME, A_NULL);
  class_addbang(counter_class, counter_bang);
  class_addmethod(counter_class, (t_method)counter_reset,
                  gensym("reset"), A_NULL);
  class_addfloat(counter_class, counter_float);
  class_addmethod(counter_class, (t_method)counter_m,
                  gensym("m"),A_FLOAT, A_NULL);
  class_addmethod(counter_class, (t_method)counter_r,
                  gensym("r"),A_FLOAT, A_NULL);
  class_addmethod(counter_class, (t_method)counter_i,
                  gensym("i"),A_FLOAT, A_NULL);
}
