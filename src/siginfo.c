...
{
    post("prev xyz: %f, %f, %f\ncurr xyz: %f, %f, %f\ndist: %f", x->xcheckprev,
         x->ycheckprev, x->zcheckprev, x->xcurr, x->ycurr, x->zcurr, x->dist);
}
...
{
    siginfo_class = class_new(gensym("siginfo"), (t_newmethod)siginfo_new, 0,
                              sizeof(t_siginfo), CLASS_DEFAULT, 0);
