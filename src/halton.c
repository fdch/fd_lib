...
{
    halton_class = class_new(gensym("halton"), (t_newmethod)halton_new, 0,
                             sizeof(t_halton), 0, A_DEFFLOAT, A_NULL);

    // class_addbang(halton_class, halton_bang);
    class_addfloat(halton_class, halton_float);
    class_addmethod(halton_class, (t_method)halton_set, gensym("set"), A_FLOAT,
                    A_NULL);
}
