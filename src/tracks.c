...
static void tracks_max_distance(t_tracks *x, t_floatarg f)
{
    x->max_dist = f;
    if (x->debug)
        post("Maximum Distance: %.2f", x->max_dist);
}
    ...
    if (new_pts >= 0)
    {
        ...
        for (i = 0; i < x->prev_pts; i++)
        {
            ...
            for (j = 0; j < x->pts; j++)
            {
                if (x->x2_s[j] == 1)
                {
                    if ((dist = tracks_getdist(x, i, j)) < dist_temp &&
                        dist < x->max_dist)
                    {
                        dist_temp = dist;
                        j_tmp = j;
    ...
    else if (new_pts < 0)
    {
        ...
        { // for each new pt
            ...
            for (j = 0; j < x->prev_pts; j++)
            {
                if (x->x1_s[j] == 1)
                {
                    if ((dist = tracks_getdist(x, j, i)) < dist_temp &&
                        dist < x->max_dist)
                    {
                        dist_temp = dist;
                        j_tmp = j;
