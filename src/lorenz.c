 ...
 * [lorenz] runs the equations for the lorenz system as in
 * (https://en.wikipedia.org/wiki/Lorenz_system) developed by Fede Camara Halac
 * (fch226@nyu.edu)
...
{
    lorenz_loop(x, &x->x_x, &x->x_y, &x->x_z, x->x_inita, x->x_initb,
                x->x_initc, x->x_inith);
}
...
static void lorenz_reset(t_lorenz *x) { lorenz_type(x, x->x_type); }

static void lorenz_inita(t_lorenz *x, t_floatarg f) { x->x_inita = f; }

static void lorenz_initb(t_lorenz *x, t_floatarg f) { x->x_initb = f; }

static void lorenz_initc(t_lorenz *x, t_floatarg f) { x->x_initc = f; }

static void lorenz_inith(t_lorenz *x, t_floatarg f) { x->x_inith = f; }

static void lorenz_initx(t_lorenz *x, t_floatarg f) { norm_reset(&x->x_x, f); }

static void lorenz_inity(t_lorenz *x, t_floatarg f) { norm_reset(&x->x_y, f); }

static void lorenz_initz(t_lorenz *x, t_floatarg f) { norm_reset(&x->x_z, f); }
...
static void lorenz_obj_reset(t_lorenz_obj *x) { lorenz_reset(x->x_lorenz); }
...
static void lorsig_reset(t_lorsig *x) { lorenz_reset(x->x_lorenz); }
...
static t_float lor_mix(t_float x1, t_float x2) { return x1 + x2 * 0.001; }
...
{
    dsp_add(lor_perform, 8, x, sp[0]->s_vec, sp[1]->s_vec, sp[2]->s_vec,
            sp[3]->s_vec, sp[4]->s_vec, sp[5]->s_vec, (int)sp[0]->s_n);
}
static void lor_inita(t_lor *x, t_floatarg f) { lorenz_inita(x->x_lorenz, f); }

static void lor_initb(t_lor *x, t_floatarg f) { lorenz_initb(x->x_lorenz, f); }

static void lor_initc(t_lor *x, t_floatarg f) { lorenz_initc(x->x_lorenz, f); }

static void lor_inith(t_lor *x, t_floatarg f) { lorenz_inith(x->x_lorenz, f); }

static void lor_initx(t_lor *x, t_floatarg f) { lorenz_initx(x->x_lorenz, f); }

static void lor_inity(t_lor *x, t_floatarg f) { lorenz_inity(x->x_lorenz, f); }

static void lor_initz(t_lor *x, t_floatarg f) { lorenz_initz(x->x_lorenz, f); }

static void lor_type(t_lor *x, t_floatarg f) { lorenz_type(x->x_lorenz, f); }

static void lor_reset(t_lor *x) { lorenz_reset(x->x_lorenz); }
...
void lorenz_global_setup(void)
{
    ...
    class_addmethod(lorenz_obj_class, (t_method)lorenz_obj_type, gensym("type"),
                    A_FLOAT, A_NULL);
    ...
    class_addmethod(lorsig_class, (t_method)lorsig_dsp, gensym("dsp"), A_NULL);

    class_addmethod(lorsig_class, (t_method)lorsig_inita, gensym("inita"),
                    A_FLOAT, A_NULL);
    class_addmethod(lorsig_class, (t_method)lorsig_initb, gensym("initb"),
                    A_FLOAT, A_NULL);
    class_addmethod(lorsig_class, (t_method)lorsig_initc, gensym("initc"),
                    A_FLOAT, A_NULL);
    class_addmethod(lorsig_class, (t_method)lorsig_inith, gensym("inith"),
                    A_FLOAT, A_NULL);
    class_addmethod(lorsig_class, (t_method)lorsig_initx, gensym("initx"),
                    A_FLOAT, A_NULL);
    class_addmethod(lorsig_class, (t_method)lorsig_inity, gensym("inity"),
                    A_FLOAT, A_NULL);
    class_addmethod(lorsig_class, (t_method)lorsig_initz, gensym("initz"),
                    A_FLOAT, A_NULL);
    class_addmethod(lorsig_class, (t_method)lorsig_type, gensym("type"),
                    A_FLOAT, A_NULL);
    class_addmethod(lorsig_class, (t_method)lorsig_normalize,
                    gensym("normalize"), A_FLOAT, A_NULL);
    class_addmethod(lorsig_class, (t_method)lorsig_reset, gensym("reset"),
                    A_NULL);
    class_addmethod(lorsig_class, (t_method)lorsig_print_types,
                    gensym("print-types"), A_NULL);

    lor_class = class_new(gensym("lor~"), (t_newmethod)lor_new,
                          (t_method)lor_tilde_free, sizeof(t_lor), 0, A_NULL);

    CLASS_MAINSIGNALIN(lor_class, t_lor, x_f);
    class_addmethod(lor_class, (t_method)lor_dsp, gensym("dsp"), A_NULL);

    class_addmethod(lor_class, (t_method)lor_inita, gensym("inita"), A_FLOAT,
                    A_NULL);
    class_addmethod(lor_class, (t_method)lor_initb, gensym("initb"), A_FLOAT,
                    A_NULL);
    class_addmethod(lor_class, (t_method)lor_initc, gensym("initc"), A_FLOAT,
                    A_NULL);
    class_addmethod(lor_class, (t_method)lor_inith, gensym("inith"), A_FLOAT,
                    A_NULL);
    class_addmethod(lor_class, (t_method)lor_initx, gensym("initx"), A_FLOAT,
                    A_NULL);
    class_addmethod(lor_class, (t_method)lor_inity, gensym("inity"), A_FLOAT,
                    A_NULL);
    class_addmethod(lor_class, (t_method)lor_initz, gensym("initz"), A_FLOAT,
                    A_NULL);
    class_addmethod(lor_class, (t_method)lor_type, gensym("type"), A_FLOAT,
                    A_NULL);
    class_addmethod(lor_class, (t_method)lor_normalize, gensym("normalize"),
                    A_FLOAT, A_NULL);
    class_addmethod(lor_class, (t_method)lor_reset, gensym("reset"), A_NULL);
}
