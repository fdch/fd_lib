    ...
    while (n--) {
      if (IS_A_FLOAT(argv, i)) {
        ...
        SETFLOAT(&x->x_listout[i],
                 ((int)atom_getintarg(i, argc, argv) == 0) ? 0 : 1);
        i++;
      } else {
...
static void sradio_print(t_sradio *x) {
  ...
  post("num: %d\ndrawn: %d\npos: %d\nval: %d\nfoc: %d\nold: %d\nfflag: "
       "%d\nkeep:%d",
       z->x_number, z->x_drawn, x->x_pos, x->x_val, x->x_foc, x->x_old,
       x->x_focflag, x->x_keep);
  // int i;
...
static void sradio_next(t_sradio *x) { sradio_step(x, 1); }

static void sradio_prev(t_sradio *x) { sradio_step(x, -1); }
...
static void sradio_send(t_sradio *x, t_symbol *s) { radio_send(x->x_radio, s); }
...
static void sradio_open(t_sradio *x) { textbuf_open(&x->x_textbuf); }

static void sradio_close(t_sradio *x) { textbuf_close(&x->x_textbuf); }
...
void g_sradio_setup(void) {
  ...
  class_addmethod(sradio_class, (t_method)sradio_keep, gensym("keep"), A_FLOAT,
                  0);
  class_addmethod(sradio_class, (t_method)sradio_open, gensym("open"), 0);
  class_addmethod(sradio_class, (t_method)sradio_close, gensym("close"), 0);
  class_addmethod(sradio_class, (t_method)sradio_addline, gensym("addline"),
                  A_GIMME, 0);
  ...
  class_addmethod(sradio_class, (t_method)sradio_flush, gensym("flush"),
                  A_SYMBOL, A_NULL);
  class_addmethod(sradio_class, (t_method)sradio_write, gensym("write"),
                  A_GIMME, 0);
  class_addmethod(sradio_class, (t_method)sradio_read, gensym("read"), A_GIMME,
                  0);
